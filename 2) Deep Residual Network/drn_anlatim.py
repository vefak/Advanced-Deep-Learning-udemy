...
train_negative_path = "/home/makman4/Documents/data/LSIFIR/Classification/Train/neg"
...
train_positive_path = "/home/makman4/Documents/data/LSIFIR/Classification/Train/pos"
...
test_negative_path ="/home/makman4/Documents/data/LSIFIR/Classification/Test/neg"
...
test_positive_path = "/home/makman4/Documents/data/LSIFIR/Classification/Test/pos"
...
#%% visualize
plt.imshow(x_train[45424,:].reshape(64,32), cmap='gray') # 45002 ve 1001
...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(trainloader):
        ...
        optimizer.zero_grad() # weightleri sıfırlıyorum. Her adımda sıfırlamak lazım. Otomatik olmuyor
        loss.backward() # backpropagation
        optimizer.step() #step ile bu weightler güncellenir
        
        if i % 2 == 0:  #iki adımda bir göreyim. Bize kalmış
            print("epoch: {} {}/{}".format(epoch,i,total_step))

    # train  # sonuçları görme
    correct = 0
    total = 0
    with torch.no_grad(): ## no grad ->> no updating ->> yani training yok
        for data in trainloader:
            images, labels = data
            images = images.view(batch_size,1,64,32) # review reshape yapılıyor"
            images = images.float()
            ...
            outputs = model(images)
            _, predicted = torch.max(outputs.data,1)  #maximum indexi return edilecek. Prediction sonucunda
            total += labels.size(0) # toplamum sizeıma eşit olacak
            correct += (predicted == labels).sum().item()  # predicted labela eşit ise doğrudur. Sonuç True veya False dener.
                                                             # sum ile 1 veya 0 olarak toplanabilir. item ile int döner
    print("Accuracy train %d %%"%(100*correct/total))
    train_acc.append(100*correct/total)  # sonuçları kaydediyoruz
    ...
    loss_list.append(loss.item())  # loss kaydediyoruz
